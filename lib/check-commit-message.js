...
module.exports = {
    ...
    performChecks : function (sMsg, cfg) {
        ...
        this.checkMaxLineLength(asMsg, cfg.maxLineLength, errors, cfg.skipCheckAfterIndent);
        ...
        this.checkSecondLineEmpty(asMsg, cfg.forceSecondLineEmpty, errors);
    ...
    __normalizeCfg : function (cfg) {
        ...
        cfg.skipCheckAfterIndent = cfg.skipCheckAfterIndent || false;
        cfg.forceSecondLineEmpty = cfg.forceSecondLineEmpty || false;
     ...
     * @param {Array} errors
     * @param {Boolean} skipCheckAfterIndent [default false] if true, lines starting with tab or two spaces won't be checked for length
     * @return {Boolean} true if test passed
     */
    checkMaxLineLength : function (asMsg, iMaxLen, errors, skipCheckAfterIndent) {
        skipCheckAfterIndent = skipCheckAfterIndent || false;
        iMaxLen = iMaxLen || 0;
        if (iMaxLen <= 0) {
            return true;
        }
        ...
        var sLongestLine = asMsg.reduce(function (prev, curr, idx, arr) {
            // Do not take lines starting with two spaces/tab into account if user asked for that
            // Empty string is passed as second param to reduce to make sure matching line won't win
            if (skipCheckAfterIndent && curr.match(/^(  |\t)/)) {
                return prev;
            }
    ...
    /**
     * Checks if second line of the commit message, if present, is empty.<br>
     * In case of failure, pushes the failure message to $errors
     * @param {Array<String>} asMsg
     * @param {Boolean} bForceSecondLineEmpty if true, performs the check
     * @param {Array} errors
     * @return {Boolean} true if test passed
     */
    checkSecondLineEmpty : function (asMsg, bForceSecondLineEmpty, errors) {
        asMsg = asMsg || [];
        if (bForceSecondLineEmpty) {
            var secondLine = asMsg[1] || "";
            if (secondLine.length > 0) {
                errors.push("Second line of the commit message should be empty but it contains: " + asMsg[1]);
                return false;
            }
        }
        return true;
